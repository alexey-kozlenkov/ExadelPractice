...
public class SkillSetDAOImpl extends GenericDAOImpl<SkillSet, SkillSet, Long> implements ISkillSetDAO {
    ...
    public void addNewSkillToUser(Long userId, Long[] skillTypeIds, Long[] levels) {
        Query query = getSession().createSQLQuery(
                "INSERT INTO \"SKILL_SET\" (skill_type_id, user_id, level) VALUES (:skillTypeId, :userId, :level)");
    ...
    public void deleteAllSkillsForUser(Long userId) {
        Query query = getSession().createQuery("DELETE FROM SkillSet WHERE user.id=:userId");
        query.setParameter("userId", userId);
        query.executeUpdate();
    }
}
